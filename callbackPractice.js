...
// If it does, return true using the callback, if not return false.
  ...
  //Code Here 
  function contains(namesArr,name,result) {
    
    if (result(namesArr.indexOf(name) >= 0)) {
      return result(true);
    }
...
function uniq(names,uniArr){
  var filteredNames = names.filter(function filterRepeats(elem,index,self){
    return index == self.indexOf(elem);
});
return uniArr(filteredNames);
}
...
//finder user id in array
//find value of user id
// put othr things of user in callback function
